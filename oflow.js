...
FlowCalculator.prototype.calculate = function (oldImage, newImage, width, height) {
    ...
    var u, v, uu, vv;
    uu = vv = 0;
    var wMax = width - winStep - step - 1;
    var hMax = height - winStep - step - 1;
    var globalY, globalX, localY, localX;

    var averageCount;

    for (globalY = winStep + step + 1; globalY < hMax; globalY += winStep) {
        for (globalX = winStep + step + 1; globalX < wMax; globalX += winStep) {
            A2 = A1B2 = B1 = C1 = C2 = 0;
...
function VideoFlow(defaultVideoTag, zoneSize) {
    var calculatedCallbacks = [],
        ...
        loopId,
        calculator = new FlowCalculator(zoneSize || 8),
        //calculator = new FlowCalculator(zoneSize || 256)
...
function WebCamFlow(defaultVideoTag, zoneSize) {
    var videoTag,
        ...
        gotSources = function(media_sources) {
